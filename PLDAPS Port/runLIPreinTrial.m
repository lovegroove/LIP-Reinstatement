...
% trial time
    ...
    % 0) VIRGIN TRIAL - load images/make textures
    dv = virginTrial(dv);
    ...
    function dv = breakFixation(dv)
        if dv.trial.state == dv.states.BREAKFIX
            ...
            if dv.trial.ttime > dv.trial.timeBreakFix + dv.trial.breakFixPenalty
                ...
                dv.trial.flagNextTrial = true; % currently breaking fix increments the trial so you would miss the presentation of certain pairs***************************** could set it to jsut go back to fixOnFunc after penalty so you would still get the same trial
                end
    ...
    function dv = virginTrial(dv)
        if dv.trial.virgin % change to virgin trial state?????? for consistancy but this might make more trouble
            ...
            if strcmp(dv.trialType, 'study')
                ...
                dv.trial.destRect = CenterRectOnPointd(dv.trial.baseRect .* dv.pa.objectSize, dv.pa.xCenter, dv.pa.yCenter); %Set size and location (find a better way to do size manipulations), also there might parameters in the ds or disp for screen center etc.
    ...
    function dv = delay(dv)
        if dv.trial.state == dv.states.DELAY
            if dv.trial.delayTime == 0 % reset timer
                dv.trial.delayTime = 0; % needs to be initialized here but not reset every time through but will this work if it has no value before this func
            end
            dv.trial.delayTime = GetSecs - dv.trial.delayTime;
            
            if fixationheld(dv) % need a grace time for them to look back at the center!!!!!!!

function [Dx, Dy] = calcVisAng(alpha, Z, scrX, scrY, screenXpixels, screenYpixels)
% Visual Angle calculations
% [Dx, Dy] = calcVisAng(alpha, Z, scrX, scrY)  % physical screen size in
% inch, Z in cm, alpha in degrees, will make measurements uniform
% eventually
...
% scrY = 9 * 2.54;
Dx = Z * screenXpixels / (scrX * 2.54) * tan(alpha*(pi/180)); % Get pixel dimensions from psychtoolbox call, also uses pixels per cm here, necessary?
Dy = Z * screenYpixels / (scrY * 2.54) * tan(alpha*(pi/180));
end
